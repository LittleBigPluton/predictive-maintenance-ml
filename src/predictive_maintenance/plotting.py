...
def plot_cv_comparison(cv_results: pd.DataFrame) -> Figure:
    """Plot mean cross-validated Average Precision with fold variability."""
    ordered_results = cv_results.sort_values(
    ...
    axis.barh(
        ...
        ordered_results["CV Average Precision"],
        xerr=ordered_results["CV Average Precision Std"],
        capsize=3,
    )
...
def plot_threshold_tradeoff(
    ...
    """Plot precision and recall across candidate decision thresholds."""
...
def plot_calibration(
    ...
    """Plot the out-of-fold probability calibration curve."""
...
def plot_expected_cost(
    cost_curve: pd.DataFrame,
    selected_threshold: float,
    cost_minimizing_threshold: float,
) -> Figure:
    """Plot expected operational cost across decision thresholds."""
    figure, axis = plt.subplots(figsize=(8, 5))
    axis.plot(
        cost_curve["Threshold"],
        cost_curve["Expected Cost"],
    )
    axis.axvline(
        selected_threshold,
        linestyle="--",
        label=f"Recall-based = {selected_threshold:.3f}",
    )
    axis.axvline(
        cost_minimizing_threshold,
        linestyle="--",
        label=f"Cost-minimizing = {cost_minimizing_threshold:.3f}",
    )
    axis.set_xlabel("Decision threshold")
...
def plot_precision_recall_curve(
    ...
    axis.set_title(
        f"{model_name} — Test Precision-Recall Curve"
    )
    figure.tight_layout()
    return figure
